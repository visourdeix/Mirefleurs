...
abstract class Grid
{
		...
		$inactive_class = '', $enabled = true, $translate = true, $checkbox = 'cb')
	{
		...
		if ($enabled)
		{
			$html[] = '<a class="btn btn-mini' . ($active_class == 'publish' ? ' active' : '') . ($tip ? ' hasTooltip' : '') . '"';
     ...
     */
	public static function featured($value, $i, $prefix = '', $enabled = true, $checkbox = 'cb')
	{
        if (is_array($prefix))
		{
			$options = $prefix;
		...
		$states = array(
			1 => array('unfeatured',  'COM_CONTENT_FEATURED' ,'COM_CONTENT_FEATURED', 'COM_CONTENT_FEATURED', true, 'featured', 'featured'),
			0 => array('featured',  'JGLOBAL_TOGGLE_FEATURED' ,'JGLOBAL_TOGGLE_FEATURED', 'JGLOBAL_TOGGLE_FEATURED', true, 'unfeatured', 'unfeatured'),
		);
