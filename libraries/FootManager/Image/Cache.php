...
class Cache {
    ...
    public function __construct($options = array()) {
        ...
        $this->quality = \FootManager\Utilities\ArrayHelper::toObject( array(
    ...
    public function cache($image, $version = "") {
        ...
        $this->image_src = $image;//strtolower($image);
        ...
        if ($this->cached_file_exists()) {
            ...
            $this->src_filesize = filesize(\FootManager\Utilities\ImageHelper::getLocalName($this->image_src));
            $this->cached_filesize = filesize(\FootManager\Utilities\ImageHelper::getLocalName($this->cached_filename));
    ...
    private function download_image() {
        $src = \FootManager\Utilities\ImageHelper::getRemoteName($this->image_src);
        $image_resource = file_get_contents($src);
        $basename = basename($src);
        if (!stripos($basename, '.' . $this->file_extension)) {
            $basename .= '.' . $this->file_extension;
        }
        $image_location = $this->cached_image_directory . '/' . $basename;
     ...
     * @return boolean the image was successfully created or not
     */
    private function fetch_image() {
        $image_size = getimagesize(\FootManager\Utilities\ImageHelper::getRemoteName($this->image_src));
        $image_width = $image_size[0];
        $image_height = $image_size[1];
        $array =explode('/', $this->file_mime_type);
        $file_mime_as_ext = end($array);
    ...
    private function docroot_to_url($src = null) {
        ...
        if (empty($this->cached_image_url)) {
            $image_url = \FootManager\Utilities\ImageHelper::getRemoteName($src);
        } else {
    ...
    private function is_image_local() {
        ...
        if (isset($parsed_src['host']) && $_SERVER['HTTP_HOST'] === $parsed_src['host'])
            return true;
        return false;
    }
    ...
    private function set_file_mime_type() {
        $image_type = exif_imagetype(\FootManager\Utilities\ImageHelper::getRemoteName($this->image_src));
        if (!$image_type) {
            $image_type = 2;
