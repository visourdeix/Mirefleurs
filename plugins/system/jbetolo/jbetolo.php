...
class plgSystemJBetolo extends JPlugin {
                        ...
                        if (JFactory::getApplication()->isSite()) {
	                        ...
	                        if ($js) {
	                        	...
	                        	foreach ($js as $j) {
	                        		...
	                        		if (strpos($j, '+') === 0) {
	                        			$j = preg_replace('/^\+/', '', $j);
	                        			$j = jbetoloFileHelper::normalizeCall($j, true, false);
	                        			$scripts .= '<script type="text/javascript" src="'.$j.'"></script>';
	                        		}
	                        	}
	                        	if ($scripts) {
	                        		$body = JResponse::getBody();
	                        		jbetoloFileHelper::placeTags($body, $scripts, 'js', 1);
	                        		JResponse::setBody($body);
	                        	}
	                        }
                        }
        ...
        public static function doJbetolo($type) {
                ...
                $allowedIn = $type == 'jbetolo' ? self::param('allow_in') : '';
        ...
        public static function dontJbetolo($type = 'jbetolo') {
                ...
                if ($input->get('type') == 'json' || $input->get('tmpl') == 'raw') {
                	return true;
                }
			...
			$excludeComponents = self::param($type . '_exclude_components');
                ...
                $excludeURLs[] = array('option' => 'com_jreviews', 'view' => 'newlisting');
                $excludeURLs[] = array('option' => 'com_jreviews', 'url' => 'listingsedit');
