 ...
 * @since  3.2
 */
class PlgFmeventsMatchdays extends JPlugin
{
    /**
	...
	public function onGetType()
	{
		...
		return [ FM_MANAGER_COMPONENT.'.matchdays' => ["title" => 'COM_FMMANAGER_FIELD_MATCHDAYS', "icon" => "trophy"]];
	}
	...
	public function onGetCategories($types)
	{
        ...
        jimport('FMManager.library');
        $result = array();
        if(empty($types) || in_array(FM_MANAGER_COMPONENT.".matchdays", $types) || in_array("all", $types)) {
	...
	public function onGetEvents($start_date, $end_date, $types, $categories, $start = 0, $limit = 0)
	{
        ...
        if($query) {
            $query = $query->with(["stadium", "competition.tournament"])
                        ->orderBy("fm_matchdays.date")
                        ->orderBy("fm_matchdays.time");
	...
	protected function getQuery($start, $end, $types, $categories)
	{
        ...
        jimport('FMManager.library');
        if(in_array("all", $types) || in_array(FM_MANAGER_COMPONENT.'.matchdays', $types)) {
            ...
            if($teams) {
                return FMManager\Database\Models\Matchday::withoutGlobalScopes()
                        ->byMatch(0)
                        ->betweenDates($start, $end)
                        ->whereHas("competition", function($query) use($teams) {
