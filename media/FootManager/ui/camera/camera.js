...
; (function ($) {
    $.fn.camera = function (opts, callback) {
        var defaults = {
            ...
            height: '66.87%',	//here you can type pixels (for instance '300px'), a percentage (relative to the width of the slideshow, for instance '50%') or 'auto'
        ...
        fakeHover.append('<div class="camera_fullscreen_btn camera_btn"><span class="fa fa-arrows-alt"></span></div>');
        fakeHover.append('<div class="camera_download_btn camera_btn"><span class="fa fa-download"></span></div>');
        ...
        var pieID = 'pie_' + wrap.index(),
            ...
            fullscreen = $('.camera_fullscreen_btn', wrap),
            download = $('.camera_download_btn', wrap),
        ...
        function thumbnailPos() {
            if ($(thumbs).length && !$(pagination).length) {
                ...
                if (firstPos == true) {
                    $('ul', thumbs).width($('ul > li', thumbs).length * ($('ul > li', thumbs).outerWidth() + 10));
        ...
        $(fullscreen).click(function () {
            if (!wrap.hasClass('camera-fullscreen')) {
                wrap.addClass('camera-fullscreen')
            } else {
                wrap.removeClass('camera-fullscreen')
            }
        });

        $(download).click(function () {
            var idNum = parseFloat($('.cameraSlide.cameracurrent', target).index());
            var url = allImg[idNum];

            if (url != undefined) {
                var link = document.createElement('a');
                link.style = 'position: fixed; left -10000px;'; // making it invisible
                link.href = url; // forcing content type
